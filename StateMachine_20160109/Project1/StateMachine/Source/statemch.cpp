...
void StateMachine::PerformStateChanges( void )
{
	...
	while( m_stateChange != NO_STATE_CHANGE && (--safetyCount >= 0) )
	{
		...
		States( EVENT_PostEnter, 0, static_cast<int>( m_currentState ), m_currentSubstate );
	}
...
/*---------------------------------------------------------------------------*
  Name:         IsInBroadcastList

  Description:  Check if an object is in the broadcast list.
  
  Arguments:    id : an ID of an object to check against the list

  Returns:      bool.
 *---------------------------------------------------------------------------*/
bool StateMachine::IsInBroadcastList( objectID id )
{
	BroadcastListContainer::iterator i;
	for( i=m_broadcastList.begin(); i!=m_broadcastList.end(); ++i )
	{
		if( *i == id )
		{	//Already in the list - escape
			return true;
		}
	}
