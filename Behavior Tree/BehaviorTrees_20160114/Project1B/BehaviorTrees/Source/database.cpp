...
/* agent communication BEGIN (by ABLA, Abdel Rahman) */
void Database::SendMsgDelayed(float delay, MSG_Name name, objectID receiver, std::string node_name, std::string tree_name, MSG_Data& data)
{
	g_msgroute.SendMsg(delay, name, receiver, SYSTEM_OBJECT_ID, node_name, tree_name, data, false, false);
}
/* agent communication END */
