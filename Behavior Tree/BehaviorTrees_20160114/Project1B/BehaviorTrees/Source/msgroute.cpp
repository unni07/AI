	...
	bool timer, bool cc)
{
	...
	if (delay <= 0.0f)
	{	//Deliver immediately
		MSG_Object msg(g_time.GetCurTime(), name, sender, receiver, node_name, tree_name, data, timer, cc);
		RouteMsg(msg);
	}
	else
	{
		...
		//Store in delivery list
		MSG_Object * msg = new MSG_Object(deliveryTime, name, sender, receiver, node_name, tree_name, data, timer, false);
		if (m_delayedMessages.empty() || deliveryTime <= m_delayedMessages.front()->GetDeliveryTime())
		{	//Put at the front if the list is empty or the delivery time is sooner than the first entry
			m_delayedMessages.push_front(msg);
		}
