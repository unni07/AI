...
public:
	...
	MSG_Object(float deliveryTime, MSG_Name name,
		objectID sender, objectID receiver,
		std::string node_name, std::string tree_name, MSG_Data& data,
		bool timer, bool cc);
	/* agent communication END */
	...
	/* agent communication BEGIN (by ABLA, Abdel Rahman) */
	inline std::string GetNodeName(void)			{ return(m_node_name); }
	inline void SetNodeName(std::string name)			{ m_node_name = name; }

	inline std::string GetTreeName(void)			{ return(m_tree_name); }
	inline void SetTreeName(std::string name)		{ m_tree_name = name; }
	/* agent communication END */
